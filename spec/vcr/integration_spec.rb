...
RSpec.describe 'it works as expected' do
  ...
  context 'recording interactions' do
    ...
    it 'records the interaction', :test_api, :vcr do
      connection.get('/get')
      expect(File).to exist('tmp/api/test_api/ww2_httpbin_org/get/GET/no_query_empty_body.yml')
    end

    it 'records the interaction in separate cassettes', :test_api, :vcr do
      connection.get('/get')
    ...
    it 'records all api interactions in separate cassettes', :test_api, :vcr do
      connection.get('/get')
      Faraday.get('http://jsonplaceholder.typicode.com/posts/1')
      connection.get('/ip')
